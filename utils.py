...
def apply_rotary_emb(x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:
    ...
    x_complex = torch.view_as_complex(
        x.float().reshape(B, H, N, D // 2, 2).contiguous()
    )  # complex64 : [B, H, N, D/2]

    # slice & broadcast freqs to [1, 1, N, D/2]
    freqs = freqs_cis[:N, :].to(x.device)          # [N, D/2]
    freqs = freqs.unsqueeze(0).unsqueeze(0).contiguous()        # [1, 1, N, D/2]
...
def apply_rotary_emb_grouped(x: torch.Tensor, freqs_cis: torch.Tensor) -> torch.Tensor:
    ...
    freqs = freqs_cis[:N]                          # [N, D//2, 2]
    freqs = torch.view_as_complex(freqs)           # [N, D//2]
    freqs = freqs.unsqueeze(0).unsqueeze(2)        # [1, N, 1, D//2]

    # 3) broadcast in complex domain
    x_rot = x_complex * freqs                      # [B, N, H, D//2]

    # 4) back to real [B, N, H, D]
    x_out = torch.view_as_real(x_rot)              # [B,N,H,D//2,2]
